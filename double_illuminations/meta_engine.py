    ...
    for iteration, (rgb_img, hyper_img, sensitivity, illumination) in enumerate(data_loader):
        ...
        for i in range(batch_size):
            ...
            if pyramid_net_grads is None or sensitivity_net_grads is None:
                ...
                pyramid_net_grads = [x / batch_size for x in net_grads[:205]]
                sensitivity_net_grads = [x / batch_size for x in net_grads[205:]]
                ...
                pyramid_net_grads = [x + y / batch_size for (x, y) in zip(pyramid_net_grads, new_net_grads[:205])]
                sensitivity_net_grads = [x + y / batch_size for (x, y) in
                                         zip(sensitivity_net_grads, new_net_grads[205:])]
