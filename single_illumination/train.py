...
def get_args_parser():
    ...
    # parser.add_argument('--log_dir', default='logs_maxl_single_notrans_norelu_smalllr/',
    #                     help='path where to save, empty for no saving')
...
def main(args):
    ...
    if args.eval:
        mean_MRAE, mean_RMSE, mean_SAM = evaluate(model, val_loader, device, args.result_dir)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, criterion, lr_scheduler, train_data_loader, optimizer, device, epoch, writer, args.batch_size)

        # meta_train_one_epoch(model, criterion, train_data_loader, primary_optimizer, auxiliary_optimizer, device, epoch,
        #                      writer, args.batch_size, args.num_gradient)
