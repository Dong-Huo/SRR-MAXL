...
def real_evaluate(model, data_loader, device, result_dir):
    ...
    for i, (rgb_img, hyper_img, sensitivity, illumination, hyper_path) in enumerate(data_loader):
        ...
        pred_sensitivity = pred_sensitivity.squeeze().detach().cpu().numpy()
        pred_sensitivity = pred_sensitivity / np.max(pred_sensitivity)


        plt.plot(
                range(420, 730, 10), pred_sensitivity[2], 'b--',
                range(420, 730, 10), pred_sensitivity[1], 'g--',
                range(420, 730, 10), pred_sensitivity[0], 'r--'
                )

        # plt.plot(range(420, 730, 10), current_sensitivity[2], 'b--', range(420, 730, 10),
        #          current_sensitivity[1], 'g--', range(420, 730, 10), current_sensitivity[0], 'r--')

        plt.legend(['Ours b', 'Ours g', 'Ours r'], loc='best', fontsize=25)

        plt.xlabel('wavelength (nm)', fontsize=40)
        plt.ylabel('CRF', fontsize=40)

        plt.savefig(os.path.join(result_dir, image_name + ".png"))
        # plt.show()
        plt.cla()
        plt.close()
